...
int main() {
 ...
 //     
	...
	//	-0.5f * 500,  0.5f * 500,  0.5f * 500, 1.0f, 0.2f,0.5f,
	//	-0.5f * 500,  0.5f * 500, -0.5f * 500, 1.0f, 0.2f,0.5f,
	//}; //serie de vértices adapatados a una forma ortogonal
	...
	glm::mat4 projection=glm::mat4(1); //variable de tipo matriz de cuatro elementos, se llamará proyeccion
	...
	while (!glfwWindowShouldClose(window)) //define matriz de proyeccion
	{
	  ...
	  view = glm::translate(view, glm::vec3(0.0f,0.0f,-12.0f));
		...
		model = glm::scale(model, glm::vec3(1.0f,5.0f, 3.0f));
		...
		model = glm::rotate(model, 50.0f, glm::vec3(0.0f, 0.0f, 1.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(1.0f, 5.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
